...
| 上百个 Skill 全量加载 | System Prompt 膨胀、注意力稀释、工具误选 | 迁移到大规模 Skill 工程实现 |
...
当 Skill 数量增长到几十个、上百个时，不要把全部 Skill 交给一个 Agent；按 [大规模 Skill 工程实现](large-scale-skill-engineering.md) 做路由、检索和动态组装。
